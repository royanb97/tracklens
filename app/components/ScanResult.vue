...
const TIER_1 = new Set(['Advertising', 'Ad Fraud', 'Ad Motivated Tracking', 'Session Replay', 'Fingerprinting', 'Unknown High Risk Behavior', 'Malware'])
const TIER_2 = new Set(['Analytics', 'Audience Measurement', 'Third-Party Analytics Marketing', 'Action Pixels', 'Tag Manager'])

type BadgeColor = 'error' | 'warning' | 'neutral'

function categoryColor(cat: string): BadgeColor {
	if (TIER_1.has(cat)) return 'error'
	if (TIER_2.has(cat)) return 'warning'
	return 'neutral'
}

function topCategories(reqs: RequestInfo[]): string[] {
	const cats = reqs[0]?.tracker?.categories ?? []
	const t1 = cats.find(c => TIER_1.has(c))
	const t2 = cats.find(c => TIER_2.has(c))
...
const scoreExplanation = computed(() => {
	...
	if (entityCount > 0) {
		const hasInvasive = entities.some(([, reqs]) =>
			reqs[0]?.tracker?.categories.some(c => TIER_1.has(c)),
		)
						...
						>
							...
							>
								<div class="flex items-center gap-2 min-w-0">
									<svg
										class="size-3.5 text-slate-500 transition-transform duration-200 shrink-0"
										:class="{ 'rotate-90': expandedTrackers.has(entity) }"
										viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"
									>
										<path stroke-linecap="round" stroke-linejoin="round" d="M9 5l7 7-7 7" />
									</svg>
									<span class="font-medium text-sm text-white truncate">{{ entity }}</span>
									<div class="flex gap-1 shrink-0">
										<UBadge
											v-for="cat in topCategories(requests)"
											:key="cat"
											:label="cat"
								...
								<UBadge :label="String(requests.length)" color="error" variant="subtle" size="sm" class="ml-2 shrink-0" />
							</button>
